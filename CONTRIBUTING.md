# Contributing to bitlocker-forensic

Thanks for your interest in improving `bitlocker-forensic`. These crates parse
untrusted Windows BitLocker (BDE) volumes acquired from potentially
...
The BitLocker reader and analyzer ship on crates.io; see
...
BitLocker volumes and an independent oracle (e.g. `libvshadow`/`vshadowinfo`), not
