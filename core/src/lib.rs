...
//!
//! Scope of this build: the **password** (`0x2000`), **recovery-password**
//! (`0x0800`), and **clear-key** (`0x0000`, no credential) protectors over **five
//! of the six** BitLocker ciphers, each validated by a `pybde` oracle —
//! AES-128-CBC ± Elephant Diffuser (`0x8000` / `0x8002`), AES-256-CBC (`0x8003`),
//! and XTS-AES-128/256 (`0x8004` / `0x8005`). Only AES-256-CBC + Elephant Diffuser
//! (`0x8001`) is recognized-but-refused (no oracle yet); startup-key and TPM
//! protectors are out of scope for *unlock*. The metadata parser still *reports*
//! every protector and cipher it finds.
//!
